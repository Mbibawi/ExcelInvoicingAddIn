...
async function invoice(issue: boolean = false) {
    ...
    if (!workbookPath) return alert('Could not retrieve the path of the Excel workbook from the localStorage');
    const tableName = findSetting(settingsNames.invoices.tableName, stored)?.value;
    if (!tableName) return alert('Could not get the name of the Excel table from the localStorage');
    ...
    if (!templatePath) return alert('Could not get a valid Word tempalte path  from the localStorage');
    ...
    (async function showInvoiceForm() {
        ...
        function insertInvoiceForm(tableTitles: string[]) {
            if (!tableTitles || !TableRows) throw new Error('The table titles or the table rows are missing. Check the console.log for more details');
            const form = byID();
            if (!form) throw new Error('The form element was not found');
            form.innerHTML = '';
            const tableBody = TableRows.slice(1, -1);
 ...
 * @returns 
 */
function inputOnChange(index: number, inputs: InputCol[], table: any[][] | undefined, invoice: boolean): any[] | void {
