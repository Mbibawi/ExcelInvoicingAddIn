...
interface IGraphAPI {
    ...
    getWordTableRows(...args: any[]): { wordRows: string[][], totalsLabels?: string[] };
...
export class LawFirm implements ILawFirm {
    ...
    async issueInvoice() {
        ...
        async function editInvoice(tableName: string, tableTitles: string[], templatePath: string, saveTo: string, graph: m.GraphAPI) {
            ...
            async function filterExcelData(inputs: HTMLInputElement[], discount: number, lang: string, invoiceNumber: string): Promise<{ wordRows: string[][], totalsLabels: string[], clientName: string, matters: string[], adresses: string[] } | void> {
                ...
                tableRows = filterTableByInputsValues([[clientNameInput!, clientCol], [matterInput!, matterCol]], excelTable!);
        ...
        async function generate(inputs: InputCol[], row: any[] | undefined) {
            ...
            (function setMissingValues() {
                ...
                (function withNoColumn() {
                    ...
                    Ctrls.baseYear.value = getIndexYear(Ctrls.baseIndexDate.value as string);
                    Ctrls.revisionYear.value = getIndexYear(Ctrls.indexDate.value as string);
    ...
    getContentControlsValues(arg: { number: string, clientName: string, matters: string[], adress: string[], lang: string }, date: Date): [string, string][] {
        const fields: InvoiceCtrls = {
            ...
            disclaimer: {
                title: 'LabelDisclamer' + ['French', 'English'].find(el => !el.toUpperCase().startsWith(arg.lang)) || 'English',
...
export class Marianne implements IGraphAPI {
    ...
    async monthlyReport(filtered?: any[][], args?: { templatePath: string, saveTo: string, tableName: string }) {

        if (!filtered) return await this.reportFactory([1, 5, 9], this.monthlyReport);
        if (!args) return;
     ...
     * @returns 
     */
    getWordTableRows(filtered: any[][]): { wordRows: string[][]; totalsLabels?: string[]; } {
        return {
            wordRows: [['']]
        };
    };
    ...
    private async issueReport(templatePath: string, saveTo: string, tables: [string, string[][], number][], contentControls: [string, string][]) {
        const form = this.form;
    ...
    findSetting(name: string, settings: settingInput[] | undefined) { return settings?.find(setting => setting.name === name) };

    appendInputsAndLabels(id: string, label: string, type: string, form: HTMLElement): HTMLInputElement {
...
class Reports {
    ...
    private idCols = [this.cols.CaseNumber, this.cols.ReceiptDate, this.cols.RegisterNumber, this.cols.CaseYear, this.cols.CaseCourt, this.cols.ClaimantName, this.cols.DefendantName];//! those will just serve to avoid adding duplicate cases to the Set()
    ...
    async monthlyProductionReport() {
        ...
        async function report(date: Date | null) {
            ...
            const issued = excelData
                ...
                .filter(row => getMonth(dateFromExcel(row[cols.AchievementMonth])) === getMonth(date));
    ...
    async returnedCasesReport() {
        ...
        async function report() {
            const returned = excelData.filter(row => row[cols.ReturnedRegisterNbr] && row[cols.CurrrentStatus] !== status.issued);
    ...
    async generateNotices() {
        const excelData = await this.fetchExelData();
        if (!excelData) return m.throwAndAlert('Failed to fetch the data from the Excel table');
        ...
        function userForm() {
            ...
            const meetingDate: HTMLInputElement = createInput('meetingDate', 'Meeting Date'), meetingHour: HTMLInputElement = createInput('meetingHour', 'Meeting Hour');

            const [claimant, defendant, caseNbr, caseCourt, caseType] = [cols.ClaimantName, cols.DefendantName, cols.CaseCourt, cols.CaseType].map(col => createInput(`input${col}`, titles[col]));



            const addCaseBtn = createBtn();

            const inputs: InputCol[] = [[claimant, cols.ClaimantName], [defendant, cols.DefendantName], [caseNbr, cols.CaseNumber], [caseCourt, cols.CaseCourt], [caseType, cols.CaseType]];

            const getInputCol = (next: HTMLInputElement) => inputs.find(([input, col]) => input === next);
            ...
            function filterOnChange(inputCol: InputCol, data: any[][]) {
                ...
                if (!_filtered.length) return m.throwAndAlert('The value provided did not correspond to any result');
                ...
                function found() {
                    const cells = (row: any[][]) => filled.map(([input, col]) => row[col]);
            ...
            function showParties(relevant: any[][]) {
                ...
                relevant.forEach(row => {
                    const [name, adress] = [relevant[cols.ClaimantName], relevant[cols.PartyAddress]];
                    const div = document.createElement('div');
        ...
        function report(parties: [any[], { meetingDate: HTMLInputElement, meetingHour: HTMLInputElement }][]) {
            ...
            parties.forEach(([party, { meetingDate, meetingHour }]) => {
                const contentControls = _ctrls.map(ctrl => {
                    ...
                    else if (ctrl === ctrls.meetingAmPm) value = hour >= 1 && hour < 8 ? value = 'مساءً' : 'صباحاً';
    ...
    async pendingCasesReport() {
        ...
        async function report() {
            ...
            const contentControls: [string, string][] = [['RTReportingPeriod', m.getDateString(today)]];
    ...
    private async editReport(wdRows: any[][], { fileName, tableName }: { fileName: string; tableName: string }, contentControls: [string, string][]) {
    ...
    private rowID(row: any[]) { return this.idCols.map(col => row[col]).join('&') };
 ...
 */
    AddCasesToReport(newRows: any[][], myUserForm: any): void {
        ...
        function rewritten(meetingDate: HTMLInputElement, meetingHour: HTMLInputElement, meetingAmPm: HTMLInputElement) {

            const tableColumns = [cols.ClaimantName, cols.DefendantName, cols.CaseNumber, cols.CaseType, cols.CaseCourt, cols.ReceiptNumber, cols.ReceiptDate, cols.CaseYear, cols.JudiciaryCaseYear];

            tableColumns.forEach((tableColumn, i) => {
                reportCasesArray.push(['', ...visibleCells.Cells(tableColumn).Value as any[]]);
            });

            const contentControls = [meetingDate!.value, meetingHour!.value, meetingAmPm!.value].map(value => ['Ctrl title', value]);
        ...
        (function vbaLogic_Unused() {
            return;
            if (myUserForm.ComboBox_ClaimantName.text !== "" || myUserForm.ComboBox_ClaimantName.Visible === false)
        ...
        (function vbaLogic() {
            ...
            if (reportCasesArray.length === 0) {
                for (let r = 0; r <= 12; r++) { reportCasesArray[r] = []; }
            }
        ...
        // --- UI Field Mapping ---
        [myUserForm.TextBox_MeetingDate, myUserForm.TextBox_MeetingHour.text, myUserForm.ComboBox_AmPm].forEach((control, i) => {
    ...
    createInput(id: string, label: string, type: string = 'text'): HTMLInputElement {
...
export function saveSettings(ui?: IUserInterface, values?: [string, string][], get: boolean = false) {
