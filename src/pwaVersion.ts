...
async function addNewEntry(add: boolean = false) {
    ...
    (async function addEntry() {
        ...
        function missing() {
            ...
            else if (row[9]) return [5, 6,7].map(index => row[index] = 0).length < 1;//This means the amount has been provided and does not  depend on the time spent or the hourly rate. We set the values of the startTime and endTime to 0, and return false (length<1 must return false)
