...
type form = { label: string; template?: string; table?: string };
const forms = {
    addRecord: {
...
async function startReporting() {
    const workbookPath: string = localStorage.reportingPath || prompt('Please provide the path for the workbook');
    if (!localStorage.reportingPath) localStorage.reportingPath = workbookPath;
    const tableName: string = localStorage.reportingTable || prompt('Please provide the name of the table');
    if (!localStorage.reportingTable) localStorage.reportingTable = tableName;
    const accessToken = await getAccessToken() || ''
    const sessionId = await createFileCession(workbookPath, accessToken, true);
    let table = await fetchExcelTableWithGraphAPI(sessionId, accessToken, workbookPath, tableName) as string[][];
    const titles = table[0];
    const form = document.getElementById('form') as HTMLDivElement;
    const btnsContainer = document.getElementById('btns') as HTMLDivElement;
    const btns = {
    ...
    function showOperationsBtns(label: string) {
        btnsContainer.innerHTML = "";

        (function showReportingBtns() {
            if (label !== btns.report) return;
            const reporting = [
                forms.monthly,
                forms.followUp,
                forms.returned,
    ...
    function issueReport(form: form) {
        const templatePath = `baseUrl${form.template}`;

        (async function monthlyReport() {
            if (form !== forms.monthly) return;
            const [criteria1, criteria2] = await getDateCriterion() as string[];
            if (!criteria1 || !criteria2) return alert('We could not build the dates for filtering the table');
            ...
            const rows: string[][] = visibleCells.slice(1);
        ...
        (async function pendingByDateReceivedReport() {
            if (form !== forms.receptionDate) return;
        ...
        (async function pendingByDateArrivedReport() {
            if (form !== forms.arrivalDate) return;
     ...
     */
    async function pendingCasesReport(sorting: number[], form: form) {
        ...
        if (!until) return alert('You must choose a valid date');
        ...
        const checker = (row: string[]) => `${row[Columns.ReceiptNumber]}${row[Columns.ReceptionDate]}${row[Columns.RegisterNumber]}`;

        let visibleRows
            : string[][] = visibleCells.slice(1);
        ...
        function getWordTableRow(row: string[], index: number) {   
            return [
                String(index + 1),
        ...
        function getCurrentStauts(row: string[], title: string) {
            if (row[Columns.CurrrentStatus] !== title) return '';
        ...
        function getInternalNumber(row:string[], form:form) {
            let date: string;
            ...
            else date = `/${getDate(row[Columns.ReceptionDate], true)}`;//We return the year as 'yyyy'

            return `${row[Columns.ReceiptNumber] || row[Columns.RegisterNumber]}\n${date}`;
            
        }

        function getCaseNumber(row: string[]) {
        ...
        function getDate(dateString: string, year: boolean = false) {
            const date = new Date(dateString);
            if (year) return date.getFullYear().toString();
            return [date.getDate(), (date.getMonth() + 1), date.getFullYear].join('/')

        }
    }

    function showCRUDForm(crud: form) {
        const args = [titles, table.slice(1), form] as [string[], any[][], HTMLDivElement];
        const api = [workbookPath, tableName, accessToken, sessionId, true] as [string, string, string, string, boolean];
...
async function promptForUserInput(type: string, message: string): Promise<string> {
...
function addRecord([titles, body, form]: [string[], any[][], HTMLDivElement], workbookPath: string, tableName: string, accessToken: string, sessionId: string, filter: boolean) {
    ...
    function createBlock(columns: number[], css: string, form: HTMLDivElement) {
        const dates = [
            Columns.ArrivalDate,
            Columns.AssignmentDate,
            Columns.ReceptionDate,
        ...
        columns.forEach(column => {
            ...
            (function insertInput() {
                ...
                input.type = 'text';
                if (dates.includes(column)) input.type = 'date';
        ...
        function addDataList(input: HTMLInputElement, index: number, unique: any[]) {
            const list = `col${index}`;
            input.setAttribute('list', list);
            if (!unique) unique = getUniqueValues(index, body);
...
function modifyRecord([titles, body, form]: [string[], any[][], HTMLDivElement], workbookPath: string, tableName: string, accessToken: string, sessionId: string, filter: boolean) {
    ...
    function modifyRecord() {
        ...
        async function modifyRow(row: any[] | undefined, filter: boolean = false) {
            if (!row) throw new Error('The row is not valid');
            const visibleCells = await addRowToExcelTableWithGraphAPI(row, body.length - 2, workbookPath, tableName, accessToken, filter);
            if (!visibleCells)
                return alert('There was an issue with the adding or the filtering, check the console.log for more details');
...
function deleteRecord([titles, body, form]: [string[], any[][], HTMLDivElement], workbookPath: string, tableName: string, accessToken: string, sessionId: string, filter: boolean) {
    ...
    function deleteRecord() {
        ...
        async function deleteRow(row: any[] | undefined, filter: boolean = false) {
            if (!row) throw new Error('The row is not valid');
            const visibleCells = await addRowToExcelTableWithGraphAPI(row, body.length - 2, workbookPath, tableName, accessToken, filter);
            if (!visibleCells)
                return alert('There was an issue with the adding or the filtering, check the console.log for more details');
...
function getValues(index: number, inputs: HTMLInputElement[]) {
...
function displayVisibleCells(visibleCells: string[][], form: HTMLDivElement) {
...
async function editReport(form: form, accessToken: string, filePath: string, rows: any[][], contentControls: string[][]) {
    ...
    (function editTable() {
        ...
        function editCells(tableRow: Element, values: string[]) {
     ...
     */
    function setTextLanguage(parent: Element, lang: string) {
